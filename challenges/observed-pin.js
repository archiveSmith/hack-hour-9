...
function getPINs(observed) {
  ...
  const nums = [[], ['2', '4'], ['1', '3', '5'], ['2', '6'], ['1', '5', '7'],
                ['2', '4', '6', '8'], ['3', '5', '9'], ['4', '8'], ['5', '7', '9'],
                ['8', '6'], ['8']];

  const outArr = [];
  const counter = 0;

  const obsNum = observed.split('');

  obsNum.forEach((num) => {
    
  })

  while (counter < observed.length) {
    for (let i = 0; i < nums[obsNum[counter]].length; i++) {
      let pushString = '';
      nums[obsNum[counter]]
